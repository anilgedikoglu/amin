...
| Kutsal Metinler (çok-dinli okuyucu + **hepsinde birden arama**) | `features/kutsal/` | `assets/sacred/*.json` — **7 metin**: Kur'an (İslam, TR Diyanet), Tao (Taoizm, TR), Dhammapada (Budizm, TR), **İncil/Kutsal Kitap (Hristiyanlık, TR — YTC 66 kitap/31.059 ayet), Tevrat (Yahudilik, sanal `from:bible bookRange[0,5]`, TR)** — İncil/Tevrat **CC BY-ND 4.0** (Yorumsuz Türkçe Çeviri © İsmail Serinken & eBible.org, WEB esaslı; atıf zorunlu, metin değiştirilmez); **Bhagavad Gita (Hinduizm, Arnold EN), Analektler (Konfüçyüsçülük, Legge EN)** kamu malı İngilizce. **EKSİK 3** (Avesta/Guru Granth/Kojiki): temiz makine-okunur kaynak YOK. |
...
- **Android min SDK:** `21`
- **Sürüm:** `1.7.3+24` (`pubspec.yaml`) — codemagic.yaml `--build-name` de eşitle
