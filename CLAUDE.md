...
| Kutsal Metinler (çok-dinli okuyucu + **hepsinde birden arama**) | `features/kutsal/` | `assets/sacred/*.json` — **10 metin**: Kur'an (İslam, TR), Tao (Taoizm, TR), Dhammapada (Budizm, TR), **İncil (Hristiyanlık, TR — YTC), Tevrat (Yahudilik, sanal `from:bible bookRange[0,5]`, TR), Popol Vuh (Maya, TR — kendi çevirim)** = TR; **Bhagavad Gita (Hinduizm, Arnold EN), Analektler (Konfüçyüsçülük, Legge EN), Mısır Ölüler Kitabı (Antik Mısır, Budge EN), Upanişadlar (Hinduizm, Müller EN)** = kamu malı EN. İncil/Tevrat **CC BY-ND 4.0** (© İsmail Serinken & eBible.org, atıf zorunlu, metin değişmez). Avesta/Guru Granth/Kojiki temiz kaynak yok → yerine Popol Vuh/Ölüler Kitabı/Upanişadlar (Gutenberg kamu malı). |
...
- **Android min SDK:** `21`
- **Sürüm:** `1.7.4+25` (`pubspec.yaml`) — codemagic.yaml `--build-name` de eşitle
