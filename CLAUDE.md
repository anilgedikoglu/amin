...
| Kutsal Metinler (çok-dinli okuyucu + **hepsinde birden arama**) | `features/kutsal/` | `assets/sacred/*.json` — **7 metin**: Kur'an (İslam, TR Diyanet), Tao Te Ching (Taoizm, TR), Dhammapada (Budizm, TR) `lang:tr` kendi çevirimiz; **İncil/Kutsal Kitap (Hristiyanlık, KJV EN 66 kitap/31.100 ayet), Tevrat (Yahudilik, sanal `from:bible bookRange[0,5]`), Bhagavad Gita (Hinduizm, Arnold EN), Analektler (Konfüçyüsçülük, Legge EN)** `lang:en` kamu malı. **EKSİK 3** (Avesta/Guru Granth/Kojiki): temiz public-domain makine-okunur kaynak YOK (Gutenberg'de arama boş). |
...
- **7 metin (~38k ayet):** Kur'an (TR Diyanet), Tao Te Ching (TR), Dhammapada (TR) — bunlar kendi çevirimiz; İncil/Kutsal Kitap (KJV EN, 66 kitap 31.100 ayet), Tevrat (sanal, İncil'in ilk 5 kitabı), Bhagavad Gita (Arnold "Song Celestial" EN), Analektler (Legge EN) — bunlar kamu malı İngilizce. **Veri üretimi:** `scratchpad/build_bible.js` (thiagobodruk/bible en_kjv.json → format) + `scratchpad/build_eastern.js` (Gutenberg 2388 Gita / 3330 Analects parse). **EKSİK 3 (Avesta/Guru Granth Sahib/Kojiki):** temiz public-domain makine-okunur kaynak yok (Gutenberg araması boş; sadece OCR/HTML). Kalite düşmesin diye eklenmedi.
...
- **Android min SDK:** `21`
- **Sürüm:** `1.7.2+23` (`pubspec.yaml`) — codemagic.yaml `--build-name` de eşitle
