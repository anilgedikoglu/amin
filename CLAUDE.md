...
| Kutsal Metinler (çok-dinli okuyucu + **hepsinde birden arama**) | `features/kutsal/` | `assets/sacred/*.json` — **10 metin**: Kur'an (İslam, TR), **Tevrat/Eski Ahit (Yahudilik, TR — 39 kitap Yaratılış→Malaki, `tevrat.json`), İncil/Yeni Ahit (Hristiyanlık, TR — 27 kitap Matta→Vahiy, `incil.json`)**, Tao (TR), Dhammapada (TR), Bhagavad Gita (TR), Popol Vuh (TR), Mısır Ölüler Kitabı (TR) = TR; **Analektler (Legge EN), Upanişadlar (Müller EN)** = kamu malı EN. **DİKKAT:** İncil=Yeni Ahit (Matta ile başlar), Tevrat=Eski Ahit (Yaratılış→Malaki); birbirine karıştırma! Tevrat+İncil YTC **CC BY-ND 4.0** (© İsmail Serinken & eBible.org, atıf **Hakkında kartında**). Okuyucuda üstteki lisans/kaynak şeridi (`_LicenseBar`) kullanıcı isteğiyle KALDIRILDI. |
...
- **7 metin (~38k ayet):** Kur'an (TR Diyanet), Tao Te Ching (TR), Dhammapada (TR) — kendi çevirimiz; **İncil/Kutsal Kitap (TR, 66 kitap 31.059 ayet)** ve **Tevrat (sanal, İncil'in ilk 5 kitabı, TR)** = "Yorumsuz Türkçe Çeviri" (YTC), eBible `turytc`, **CC BY-ND 4.0** (© 2023–2025 İsmail Serinken & eBible.org, World English Bible esaslı — **atıf zorunlu, metin değiştirilemez**; `_LicenseBar` ve Hakkında'da atıf gösteriliyor); Bhagavad Gita (Arnold "Song Celestial" EN), Analektler (Legge EN) = kamu malı İngilizce. **Veri üretimi:** `scratchpad/build_bible_tr.js` (eBible turytc USFM zip → parse; USFM işaretleri + dipnot/xref apparatı temizlenir, ayet metni AYNEN korunur) + `build_eastern.js` (Gutenberg 2388 Gita / 3330 Analects). **EKSİK 3 (Avesta/Guru Granth/Kojiki):** temiz makine-okunur kaynak yok.
...
- **Android min SDK:** `21`
- **Sürüm:** `1.7.8+29` (`pubspec.yaml`) — codemagic.yaml `--build-name` de eşitle
